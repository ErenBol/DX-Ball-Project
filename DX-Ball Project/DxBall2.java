...
public class DxBall2 {
    public static void main(String args[]){
        ...
        Color[] colors = new Color[]{
                new Color(204, 0, 0),      // Dark Red
                new Color(255, 221, 51),   // Golden Yellow
                new Color(255, 51, 51),    // Light Red
                new Color(255, 179, 0),    // Yellow Orange
                new Color(153, 0, 0),      // Deep Red
                new Color(255, 179, 51),   // Warm Yellow
                new Color(255, 102, 102),  // Soft Red
                new Color(255, 204, 51),   // Bright Yellow
                new Color(204, 51, 51),    // Crimson Red
                new Color(255, 204, 102)   // Light Golden Yellow
        };
        ...
        Color ballColor = new Color(15, 82, 186); // Keeping the same ball color for consistency
        ...
        Color paddleColor = new Color(75, 0, 130); // Keeping the paddle color consistent
        ...
        double[] lineCoordinates = {initialBallPos[0],initialBallPos[1],initialBallPos[0]+40,initialBallPos[1]};
        ...
        while(!isGameOver) {  // checking if the game is over (victory/lose)
            while (!isPaused) { // checking if the game is paused
                if (StdDraw.isKeyPressed(37) && (paddlePos[0] > paddleHalfwidth)) { // left
                    paddlePos[0] -= paddleSpeed;
                }
                if (StdDraw.isKeyPressed(39) && (paddlePos[0] < xScale-paddleHalfwidth)) { // right
                    paddlePos[0] += paddleSpeed;
                }
    ...
    public static ArrayList<Object> whereIsCollided(double[] ballsLocation, double[][] brickLocations, double[] paddleLocations, double xVel, double yVel, int newScore, Color[] brickColors, boolean isOver, double[] dimensions){   // xVel, yVel,brickLocations,score
        ...
        for (int j = 0; j < brickLocations.length; j++) {
            ...
            if (xBall + 8 >= xBrick - brickHalfwidth &&
                ...
                if (cornerHit) {
                    ...
                    if (horizontalEdgeHit && !yVelocityChanged) {
                        yVel = -yVel;
                        yVelocityChanged = true;
                    }
                    if (verticalEdgeHit && !xVelocityChanged) {
                        xVel = -xVel;
                        xVelocityChanged = true;
                    }
                }
    ...
    public static void endGame(int score, int brickCount){
        ...
        if (score !=brickCount*10){
            StdDraw.setPenColor(StdDraw.RED);
            StdDraw.text(400,100,"Error 404: Skills Not Found!");
        }
        else{
